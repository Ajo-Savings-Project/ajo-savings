...
export function pluralize(count: number, singular: string, irregularPlural?: string, options?: PluralizeOptions) {
...
export const stringToSentenceCase = (str: string) => {
  return str.replace(/\.\s+([a-z])[^.]|^(\s*[a-z])[^.]/g, (s) => s.replace(/([a-z])/, (v) => v.toUpperCase()));
};

export const capitalizeWord = (word: string) => word.replace(/\b\w/g, (match) => match.toUpperCase());
